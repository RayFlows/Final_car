...
def cv2ImgAddText(img, label, left, top, textColor=(255, 255, 255)):
    ...
    #   设置字体
    #---------------#
    font = ImageFont.truetype(font='facenet_retinaface_pytorch/simhei.ttf', size=48)
...
class Retinaface(object):
    _defaults = {
        ...
        #   retinaface训练完的权值路径
        #----------------------------------------------------------------------#
        "retinaface_model_path" : 'facenet_retinaface_pytorch/model_data/Retinaface_mobilenet0.25.pth',
        #----------------------------------------------------------------------#
        ...
        #   facenet训练完的权值路径
        #----------------------------------------------------------------------#
        "facenet_model_path"    : 'facenet_retinaface_pytorch/model_data/facenet_mobilenet.pth',
        #----------------------------------------------------------------------#
    ...
    def __init__(self, encoding=0, **kwargs):
        ...
        try:
            self.known_face_encodings = np.load("facenet_retinaface_pytorch/model_data/{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone))
            self.known_face_names     = np.load("facenet_retinaface_pytorch/model_data/{backbone}_names.npy".format(backbone=self.facenet_backbone))
        except:
            if not encoding:
    ...
    def encode_face_dataset(self, image_paths, names):
        ...
        for index, path in enumerate(tqdm(image_paths)):
            ...
            # 2. ★★ 新增：统一转成 RGB ★★
            if image.mode == 'RGBA':        # 4 通道 PNG
                image = image.convert('RGB')
            elif image.mode == 'L':         # 灰度
                image = image.convert('RGB')

            # 3. 再转成 numpy，后续流程保持不变
        ...
        np.save("facenet_retinaface_pytorch/model_data/{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone),face_encodings)
        np.save("facenet_retinaface_pytorch/model_data/{backbone}_names.npy".format(backbone=self.facenet_backbone),names)

    #---------------------------------------------------#
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image, return_info=False):
        ...
        scale_for_landmarks = scale * 2 + [im_width, im_height]
        ...
        with torch.no_grad():
            image_tensor = torch.from_numpy(preprocess_input(image).transpose(2, 0, 1)).unsqueeze(0).float()
            if self.cuda:
        ...
        for b in boxes_conf_landms:
            b = np.maximum(b, 0)
            crop_img = old_image[int(b[1]):int(b[3]), int(b[0]):int(b[2])]
        ...
        for i, b in enumerate(boxes_conf_landms):
            ...
            # 填充返回结构
            boxes_ret.append(b[0:4])
            probs_ret.append(score)
            names_ret.append(name)

            # 绘制
            cv2.rectangle(old_image, (b[0], b[1]), (b[2], b[3]), (0, 0, 255), 4)
            cv2.putText(old_image, f"{score:.2f}", (b[0], b[1] + 30),
        ...
        self.last_face_names = names_ret

        if return_info:
            return old_image, boxes_ret, probs_ret, names_ret
        else:
            return old_image

            # Ensure input is 3-channel RGB
            if isinstance(image, Image.Image):
                # 如果传入是 PIL.Image（用于兼容性）
                if image.mode == 'RGBA':
                    image = image.convert('RGB')
                elif image.mode == 'L':
                    image = image.convert('RGB')
                ...
                # 如果是 numpy 数组（比如摄像头帧），也检查通道数
                if image.shape[2] == 4:
                    image = image[:, :, :3]  # 丢掉 Alpha 通道
            #---------------------------------------------------#
            #   对输入图像进行一个备份，后面用于绘图
            #---------------------------------------------------#
            old_image   = image.copy()
