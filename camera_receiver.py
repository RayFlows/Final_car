...
#                 start_time = time.time()  # 调试：记录处理时间
...
#                     # 修改：初始化 processed_frame 并绘制绿框和姿态文本
#                     processed_frame = frame.copy()
#                     cv2.rectangle(processed_frame, (50, 50), (400, 400), (0, 255, 0), 2)
#                     cv2.putText(processed_frame, f"Posture: {posture}", (50, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

#                 if not frame_queue.full():
#                     frame_queue.put(processed_frame)

#                 print(f"帧处理时间: {time.time() - start_time} 秒")  # 调试：输出处理时间
...
# 设置项目根目录（Final_car 的根）
PROJECT_DIR = os.path.abspath(os.path.dirname(__file__))
sys.path.insert(0, PROJECT_DIR)

# 确保包含 'Infant-Pose-Estimation' 的路径
pose_estimation_path = os.path.join(PROJECT_DIR, 'Infant-Pose-Estimation')
pose_path = os.path.join(pose_estimation_path, 'tools')
sys.path.insert(0, pose_path)
...
print(f"Pose tools path: {pose_path}")
...
# 新增：跳帧计数器和上次姿态缓存
SKIP_FRAMES = 30  # 每3帧预测一次，可调整为更大值以进一步优化
pose_frame_counter = 0
last_posture = "Unknown"  # 初始化默认姿态

pose_model_path = os.path.join(PROJECT_DIR, 'Infant-Pose-Estimation', 'models', 'hrnet_fidip.pth')
print(f"Pose model path in camera_receiver: {pose_model_path}")
...
def run():
    global last_frame, processed_frame, pose_frame_counter, last_posture
    print("启动视频接收器...")
        ...
        while True:
            ...
            if frame is not None:
                ...
                if mode == "cruise":
                    ...
                    print(f"当前模式：{mode}，绘制红框")  # 保留原有打印
                    # cv2.rectangle(processed_frame, (50, 50), (400, 400), (0, 0, 255), 2)

                elif mode == "pose":
                    # 修改：添加跳帧逻辑，只每 SKIP_FRAMES 帧运行完整预测
                    pose_frame_counter += 1
                    if pose_frame_counter >= SKIP_FRAMES:
                        posture = run_pose_prediction(frame, device)  # 直接传递 frame (numpy array)
                        last_posture = posture
                        socketio.emit('update_prediction', {'posture': posture})
                        print(f"当前模式：{mode}，姿势预测结果: {posture}")  # 只在预测时打印
                        pose_frame_counter = 0  # 重置计数器
                    else:
                        posture = last_posture  # 复用上次结果
                        # print(f"复用上次姿态: {posture}")  # 可选调试

                    # 初始化 processed_frame 并绘制
                    processed_frame = frame.copy()
                    cv2.rectangle(processed_frame, (50, 50), (400, 400), (0, 255, 0), 2)
                    cv2.putText(processed_frame, f"Posture: {posture}", (50, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                ...
                print(f"帧处理时间: {time.time() - start_time} 秒")
