...
# mlp_model_path = os.path.join(PROJECT_DIR, "Infant-Pose-Estimation", "classifier", "pose_classifier.pth")
# config_path = os.path.join(PROJECT_DIR, "Infant-Pose-Estimation", "experiments", "coco", "hrnet", "w48_384x288_adam_lr1e-3_custom.yaml")

# print(f"全局加载配置: {config_path}")
...
# print("全局加载姿势估计模型...")
# GLOBAL_POSE_MODEL = get_pose_net(cfg, is_train=False)
# state = torch.load(pose_model_path, map_location=GLOBAL_DEVICE)
# GLOBAL_POSE_MODEL.load_state_dict(state, strict=False)
# GLOBAL_POSE_MODEL.to(GLOBAL_DEVICE).eval()
# print("姿势估计模型加载完成。")

# print("全局加载 MLP 分类器模型...")
# GLOBAL_MLP_MODEL = MLPClassifier(input_size=34, hidden_size=256, output_size=4)
# state = torch.load(mlp_model_path, map_location=GLOBAL_DEVICE)
# GLOBAL_MLP_MODEL.load_state_dict(state)
# GLOBAL_MLP_MODEL.to(GLOBAL_DEVICE).eval()
# print("MLP 分类器模型加载完成。")
...
#     posture = predict_pose(image_path, cfg, device)
#     return posture
...
# #     return image
# def draw_pose(image):
#     keypoints = get_keypoints(image, GLOBAL_POSE_MODEL, cfg, GLOBAL_DEVICE)
...
# 设置项目根目录（Infant-Pose-Estimation 的根）
PROJECT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
sys.path.insert(0, PROJECT_DIR)

# 显式添加 lib 和 lib/models 到 sys.path 以确保导入
LIB_DIR = os.path.join(PROJECT_DIR, 'lib')
MODELS_DIR = os.path.join(PROJECT_DIR, 'lib', 'models')
...
# 打印 sys.path 以调试
print("sys.path in pose_estimation.py:", sys.path)

from lib.models.pose_hrnet import get_pose_net
from lib.core.inference import get_final_preds
from lib.utils.transforms import get_affine_transform
from classifier.mlp_classifier import MLPClassifier
...
if not os.path.exists(mlp_model_path):
    raise FileNotFoundError(f"MLP 模型文件不存在: {mlp_model_path}")
...
print(f"全局加载姿势估计模型: {pose_model_path}")
GLOBAL_POSE_MODEL = get_pose_net(cfg, is_train=False)
state = torch.load(pose_model_path, map_location=GLOBAL_DEVICE)
GLOBAL_POSE_MODEL.load_state_dict(state, strict=False)
GLOBAL_POSE_MODEL.to(GLOBAL_DEVICE).eval()
...
print(f"全局加载 MLP 分类器模型: {mlp_model_path}")
GLOBAL_MLP_MODEL = MLPClassifier(input_size=34, hidden_size=256, output_size=4)
state = torch.load(mlp_model_path, map_location=GLOBAL_DEVICE)
GLOBAL_MLP_MODEL.load_state_dict(state)
GLOBAL_MLP_MODEL.to(GLOBAL_DEVICE).eval()
...
# 新增：树莓派控制配置（参考 auto_controller.py）
...
def send_control_command(cmd):
    try:
        control_sock.sendto(cmd.encode(), (RASPBERRY_IP, CONTROL_PORT))
        print(f"已发送控制指令到树莓派: {cmd}")
    except Exception as e:
        print(f"发送控制指令失败: {e}")

# 图像预处理
# 修改：图像预处理（接受 img 数组，而不是路径）
...
# 获取图像的关键点
def get_keypoints(img, model, cfg, device):
    print(f"开始提取关键点（内存中）")
    inp_tensor, center, scale, orig_bgr = preprocess_image(img, cfg)
    inp_tensor = inp_tensor.to(device)
...
# 姿势预测函数
def predict_pose(img, cfg, device):
    print(f"开始进行姿势预测（内存中）")
    keypoints = get_keypoints(img, GLOBAL_POSE_MODEL, cfg, device)
    keypoints_xy = [(float(x), float(y)) for x, y in keypoints]  # 将 keypoints 转换为原生 Python float 类型

    posture = classify_pose_with_mlp(keypoints_xy, GLOBAL_MLP_MODEL, device)

    posture_dict = {0: "Sitting", 1: "Supine", 2: "Prone", 3: "Standing"}

    # print(f"预测的姿势标签: {posture_dict.get(posture, 'Unknown')}")
    # return posture_dict.get(posture, 'Unknown')
    predicted_posture = posture_dict.get(posture, 'Unknown')  # 修改：保存预测结果以便检查
    print(f"预测的姿势标签: {predicted_posture}")

    # 新增：如果检测到 Prone 姿势，发送 "l" 指令
    if predicted_posture == "Prone":
        send_control_command("l")

    return predicted_posture

# 运行姿势预测（简化版，使用全局模型）
def run_pose_prediction(img, device=None):
    print("开始运行姿势预测...")
    device = device or GLOBAL_DEVICE
    posture = predict_pose(img, cfg, device)
    return posture
