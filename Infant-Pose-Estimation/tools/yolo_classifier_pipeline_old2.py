...
def detect_and_predict_from_camera(pose_model, mlp_model, cfg, device):
    ...
    def classify_from_queue():
        while True:
            frame = frame_queue.get()  # 获取队列中的帧
            if frame is None:  # 如果接收到停止信号
                break
            # 对帧中的婴儿图像进行分类
    ...
    # 启动一个线程来处理分类
    threading.Thread(target=classify_from_queue, daemon=True).start()
