...
def validate_feature(config, val_loader, val_dataset,
                     model_p, criterion_p,
                     output_dir, tb_log_dir, writer_dict=None):

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # ★修改
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()

    model_p.to(device)        # ★修改
    model_p.eval()
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_weight, meta) in enumerate(val_loader):
            ...
            # 取最后一层
            if isinstance(outputs, list):
                output, feature_output = outputs[-1], feature_outputs[-1]
            else:
                output, feature_output = outputs, feature_outputs

            # 保存特征
            feature_np = feature_output.cpu().numpy()
            feature_array = feature_np if i == 0 else np.concatenate((feature_array, feature_np))

            # ★修改：FLIP_TEST 时也统一 device
            if config.TEST.FLIP_TEST:
                input_flipped = torch.flip(input, dims=[3])
                feature_flipped, outputs_flipped = model_p(input_flipped)
                ...
                output_flipped = torch.from_numpy(output_flipped.copy()).to(device)  # ✅ 修复 stride 报错
            ...
            if i % config.PRINT_FREQ == 0:
                ...
                prefix = f'{os.path.join(val_dir, "val")}_{i}'
                save_debug_images(config, input.cpu(), meta, target.cpu(), pred*4, output.cpu(), prefix)
